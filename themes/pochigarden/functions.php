...
function twentytwelve_widgets_init() {
	register_sidebar( array(
	 ...
	 'id' => 'footer-left',
	 'description' => 'フッター左',
	 'before_widget' => '<div class="col-md-4" role="complementary" id="execphp-6">',
	 'after_widget' => '</div>',
	 'before_title' => '<h3 class="widgettitle">',
	 'after_title' => '</h3>'
	  ));
	 ...
	 'id' => 'footer-center',
	 'description' => 'フッター中央',
	 'before_widget' => '<div class="col-md-4 m-archive" role="complementary" >',
	 'after_widget' => '</div>',
	 'before_title' => '<h3 class="widgettitle">',
	 'after_title' => '</h3>'
	  ));
	 ...
	 'id' => 'footer-right',
	 'description' => 'フッター右',
	 'before_widget' => '<div class="col-md-4 " role="complementary">',
	 'after_widget' => '</div>',
	 'before_title' => '<h3 class="widgettitle">',
	 'after_title' => '</h3>'
	  ));
...
//自動作成する画像サイズ
add_image_size('size60_60',60,60,true);//サムネイル
add_image_size('size250_160',250,160,true); //アーカイブノート写真
add_image_size('size230_160',230,160,true); //WIKI−２写真
add_image_size('size175_160',175,160,true);//WIKI写真
//add_image_size('size600_350',600,350,true);
...
function articleFunc($atts) {
	...
	$post = get_posts('post_status=publish&numberposts='.$numberposts.'&offset='.$offset.'&order='.$order.'&orderby='.$orderby.'&include='.$include.'&year='.$y.'&monthnum='.$m.'&day='.$d.'&exclude='.get_the_ID().','.$exclude.'&meta_key='.$meta_key.$mode.'&post_type='.$post_type);
	...
	foreach ($post as $item){
		...
		<div class="col-md-12 box250">
			...
			<h5><a href="'.get_permalink($item->ID).'">'.$item->post_title.'</a></h5>
			
			<p class="photo"> 
			'. get_the_post_thumbnail($item->ID, 'size230_160')  .'
			</p>
			
			<ul>
				<li class="date">'.get_the_date('',$item->ID).'</li>
				<p>'.get_the_excerpt($item->ID).'</p>
				
			</ul>
			<p class="more-link">
				<a href="'.get_permalink($item->ID).'" title="｢'.get_permalink($item->ID).'の続きを読む">続きを読む　&raquo;</a>
			</p>
		...
		$a = new WP_Query($args);
			if($a->have_posts()):
				...
				while($a->have_posts()):
					...
					if(($post_count % 4 )== 1 && $post_count != 1) // ４で割った余りが１で、なおかつカウンターが１（最初）でなければ閉じulと囲みの開始タグを出力		
					{
						$str.='</ul>'."\n".'<ul class="row">'."\n";
...
function blog_posts_type( $wp_query ) {
 ...
 $wp_query->set( 'posts_per_page', 10 ); // 10件ずつ表示
   ...
   elseif ( $wp_query->is_main_query() && is_tax() ||is_search())   { //管理画面以外 かつ メインクエリー かつ タクソノミー
...
add_action( 'pre_get_posts', 'foo_modify_main_query' );//うまくうごかず。taxonomy.phpで固定ページ抜きたい。
function foo_modify_main_query( $query ) {
  if ( is_admin() || ! $query->is_main_query() )
    return;

  if ( $query->is_tax(  ) ) { 
    $query->set( 'post_type', array( 'post','plants','biotope' ) ); // 投稿とカスタム投稿タイプ news を含める
    return;
  }
 }
